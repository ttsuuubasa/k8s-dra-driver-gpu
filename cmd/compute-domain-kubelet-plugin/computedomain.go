...
func (m *ComputeDomainManager) onAddOrUpdate(ctx context.Context, obj any) error {
	...
	cd, err := m.GetComputeDomain(ctx, string(cd.UID))
	if err != nil {
		return fmt.Errorf("error getting ComputeDomain: %w", err)
	}
	if cd == nil {
...
func (m *ComputeDomainManager) startComputeDomainSettings(ctx context.Context, cdUID, rcName, rcNamespace string) error {
	...
	case errors.Is(err, ErrBindingFailure):
		...
		if err := m.RemoveResourceClaimName(ctx, cdUID, rcName); err != nil {
			return fmt.Errorf("error removing ResourceClaim Name in ComputeDomain: %w", err)
		}
...
func (m *ComputeDomainManager) RemoveResourceClaimName(ctx context.Context, cdUID, rcName string) error {
	cd, err := m.GetComputeDomain(ctx, cdUID)
	if err != nil {
		return fmt.Errorf("error getting ComputeDomain: %w", err)
	}
	...
	for _, rc := range newCD.Status.ResourceClaims {
		if rc.Name == rcName {
			continue
		}
